...
.stTextInput input,.stDateInput input,.stNumberInput input,.stTextArea textarea,.stSelectbox div[data-baseweb="select"]>div{background:#181d26!important;color:#e8edf2!important;border:1px solid rgba(255,255,255,.10)!important;border-radius:14px!important}
...
def init_state():
    defaults = {"authenticated": False, "useremail": "", "lang": "EN", "analyticsview": "purchase"}
    for k, v in defaults.items():
        if k not in st.session_state:
            st.session_state[k] = v


def require_swag_config():
    cfg = st.secrets.get("SWAG")
    if not cfg:
        st.error("SWAG section missing in secrets.toml")
        return None
    for key in ["url", "db", "user", "apikey"]:
        if key not in cfg:
            st.error(f"SWAG.{key} missing in secrets.toml")
            return None
    return cfg
...
def fetch_swag_purchase_history(modelcode, datefrom, dateto):
    ...
    if not uid:
        return empty
    try:
        domain = [["order_id.state", "in", ["purchase", "done"]], ["order_id.date_order", ">=", f"{datefrom} 00:00:00"], ["order_id.date_order", "<=", f"{dateto} 23:59:59"]]
        ...
        for line in lines:
            ...
            rows.append([datestr, str(order.get("name") or ""), vendor, bc, category, str(prod.get("default_code") or ""), str(prod.get("display_name") or ""), qty, unit, round(qty * unit, 2)])
        return pd.DataFrame(rows, columns=cols).sort_values(by=cols[0], ascending=False).reset_index(drop=True)
    except Exception as e:
        st.error(f"Purchase fetch error: {e}")
        return empty
...
def fetch_swag_sales_history(modelcode, datefrom, dateto):
    ...
    if not uid:
        return empty
    try:
        domain = [["order_id.state", "in", ["sale", "done"]], ["order_id.date_order", ">=", f"{datefrom} 00:00:00"], ["order_id.date_order", "<=", f"{dateto} 23:59:59"]]
        if modelcode and str(modelcode).strip():
            domain.append(["product_id.default_code", "ilike", str(modelcode).strip()])
        ...
        for line in lines:
            ...
            unit = float(line.get("price_unit") or 0)
            rows.append([datestr, str(order.get("name") or ""), customer, bc, category, str(prod.get("default_code") or ""), str(prod.get("display_name") or ""), qty, unit, round(qty * unit, 2)])
        return pd.DataFrame(rows, columns=cols).sort_values(by=cols[0], ascending=False).reset_index(drop=True)
    except Exception as e:
        st.error(f"Sales fetch error: {e}")
        return empty
...
def render_login():
    ...
    with c2:
        ...
        if lg != get_lang():
            ...
            st.rerun()
    st.markdown("""
    <div class='hero'><div><div class='badge'>SWAG INTELLIGENCE PLATFORM</div><h1>Real-Time <span>Sales & Purchase</span><br>Intelligence</h1><p>Unified analytics for the SWAG Odoo system. Track sales history, purchase history, customer trends, vendor trends, categories, and model-level performance in one premium dashboard.</p><div class='system-row'><div class='system-chip'>📈 Sales Analytics</div><div class='system-chip'>🛒 Purchase Analytics</div><div class='system-chip'>📦 Date-based Records</div><div class='system-chip'>⬇️ Excel / CSV Export</div></div><div class='system-row'><div class='system-chip'>SWAG (Main)</div><div class='system-chip'>Odoo XML-RPC</div><div class='system-chip'>EN / AR</div></div></div><div class='preview'><div class='preview-top'><span class='dot r'></span><span class='dot y'></span><span class='dot g'></span></div><div class='preview-grid'><div class='preview-stat'><div class='v'>Sales</div><div class='l'>History & trends</div></div><div class='preview-stat'><div class='v'>Purchase</div><div class='l'>Vendor visibility</div></div><div class='preview-stat'><div class='v'>Export</div><div class='l'>Excel / CSV</div></div><div class='preview-stat'><div class='v'>AR / EN</div><div class='l'>Bilingual UI</div></div></div><div class='preview-bars'><span style='height:42%'></span><span style='height:68%'></span><span style='height:56%'></span><span style='height:84%'></span><span style='height:52%'></span><span style='height:92%'></span><span style='height:70%'></span><span style='height:80%'></span></div></div></div>
    """, unsafe_allow_html=True)
...
def sales_view():
    ...
    if df.empty:
        ...
        return
    qty_col, sub_col, cust_col, cat_col = t("Qty", "الكمية"), t("Subtotal", "الإجمالي"), t("Customer", "العميل"), t("Category", "الفئة")
    render_kpis([
    ...
    with d2:
        st.download_button(t("Download Excel", "تحميل Excel"), to_excel(df, "SWAG Sales"), file_name=f"swag_sales_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
...
def purchase_view():
    ...
    if df.empty:
        ...
        return
    qty_col, sub_col, ven_col, cat_col = t("Qty", "الكمية"), t("Subtotal", "الإجمالي"), t("Vendor", "المورد"), t("Category", "الفئة")
    render_kpis([
        kpi_card(f"{len(df):,}", t("Rows", "الصفوف")),
        kpi_card(f"{df[qty_col].sum():,.0f}", t("Total Qty", "إجمالي الكمية")),
    ...
    with d2:
        st.download_button(t("Download Excel", "تحميل Excel"), to_excel(df, "SWAG Purchase"), file_name=f"swag_purchase_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
...
def dashboard():
    with st.sidebar:
        ...
        if st.button(t("Logout", "تسجيل الخروج"), use_container_width=True):
            logout()
    st.markdown(f"<div class='hero'><div><div class='badge'>{t('LIVE DASHBOARD · SWAG ONLY', 'لوحة مباشرة · SWAG فقط')}</div><h1>{t('Sales & Purchase', 'المبيعات والمشتريات')} <span>{t('Analytics', 'التحليلات')}</span></h1><p>{t('Track SWAG sales and purchase history with date filters, top customers, top vendors, categories, detailed tables, and export-ready reports in one premium workspace.', 'تابع تاريخ المبيعات والمشتريات لنظام SWAG مع فلاتر التاريخ وأعلى العملاء وأعلى الموردين والفئات والجداول التفصيلية والتقارير الجاهزة للتصدير في مساحة عمل احترافية.')}</p><div class='system-row'><div class='system-chip'>SWAG Odoo</div><div class='system-chip'>{t('Sales', 'المبيعات')}</div><div class='system-chip'>{t('Purchase', 'المشتريات')}</div><div class='system-chip'>{t('Exports', 'التصدير')}</div></div></div><div class='preview'><div class='preview-top'><span class='dot r'></span><span class='dot y'></span><span class='dot g'></span></div><div class='preview-grid'><div class='preview-stat'><div class='v'>1</div><div class='l'>SWAG System</div></div><div class='preview-stat'><div class='v'>2</div><div class='l'>Analytics Views</div></div><div class='preview-stat'><div class='v'>CSV</div><div class='l'>Downloads</div></div><div class='preview-stat'><div class='v'>XLSX</div><div class='l'>Reports</div></div></div><div class='preview-bars'><span style='height:45%'></span><span style='height:65%'></span><span style='height:58%'></span><span style='height:88%'></span><span style='height:54%'></span><span style='height:90%'></span><span style='height:74%'></span><span style='height:82%'></span></div></div></div>", unsafe_allow_html=True)
